...
class Router(Node):
    ...
    def __init__(self, name, caching_policy='LRU', alpha=0.9):
        ...
        self.popularity_table = pd.DataFrame(columns=['Content Name', 'R_count', 'Popularity', 'Rank'])
    ...
    def update_popularity(self, content_name):
        """Update the request count and popularity score for content."""
        if content_name in self.popularity_table['Content Name'].values:
            ...
            new_popularity = self.alpha * current_popularity + (1 - self.alpha) * r_count
            self.popularity_table.at[content_index, 'R_count'] = r_count
            self.popularity_table.at[content_index, 'Popularity'] = new_popularity
        else:
            # Add new content entry with initial values if it doesn't already exist
            if not self.popularity_table.empty or any(pd.notna(self.popularity_table)):
                new_entry = {'Content Name': content_name, 'R_count': 1, 'Popularity': 1 - self.alpha}
                new_df = pd.DataFrame([new_entry])
                self.popularity_table = pd.concat([self.popularity_table, new_df], ignore_index=True)
            self.rank_content()
    ...
    def save_popularity_table(self, policy):
        """Save the popularity table to a policy-specific CSV."""
...
def setup_network():
    ...
    # If no existing network is used, create a new one
...
# Load dataset
policy_stats_path = "Policy_Stats/LRU_stats.csv"  # Change if needed
df = pd.read_csv(policy_stats_path)
df = df.dropna()  # Remove missing values
df = df[df['Cache Hit Ratio'] >= 0]  # Keep only valid entries
...
# Convert Cache Hit Ratio into a binary target (High = 1 if above 50%, else Low = 0)
df['High_Cache_Hit'] = (df['Cache Hit Ratio'] > df['Cache Hit Ratio'].mean()).astype(int)  # Lower threshold to ensure two classes
print("Unique values in High_Cache_Hit:", df['High_Cache_Hit'].value_counts())

if df['High_Cache_Hit'].nunique() < 2:
    print("Warning: The dataset contains only one class. Consider modifying the simulation parameters.")
    df['High_Cache_Hit'] = (df['Cache Hit Ratio'] > 0).astype(int)  # Ensure at least some 1s


# Define features and target
X = df[['No of Clients', 'Total Requests', 'Hop Reduction']]
y = df['High_Cache_Hit']
...
# Function to predict cache hit probability
def predict_cache_hit(no_of_clients, total_requests, hop_reduction):
    input_data = np.array([[no_of_clients, total_requests, hop_reduction]])
    input_scaled = scaler.transform(input_data)
    prediction = model.predict(input_scaled)
    probability = model.predict_proba(input_scaled)[:, 1]  # Probability of High Cache Hit
...
total_requests = 50
hop_reduction = 0.3
pred, prob = predict_cache_hit(no_of_clients, total_requests, hop_reduction)
print(f"Predicted High Cache Hit: {pred}, Probability: {prob:.2f}")
...
def run_simulation(routers, publishers, subscribers, policy, iterations):
    """Simulate content requests and measure caching policy performance."""
    ...
    avg_latency = sum(router.total_cache_access_time for router in routers) / total_requests if total_requests > 0 else 0
    ...
    return {
        "Policy": policy,
        "Simulation Data": [[0, 0, total_requests, total_hop_reduction, avg_cache_hit, avg_latency]],
        "Cache Hit Ratio": avg_cache_hit
    }

# Now, we run the simulation for each caching policy
...
for policy in policies:
    ...
    policy_stats.append({
        "Policy": policy,
        "Cache Hit Ratio": stats["Cache Hit Ratio"],
        "Latency": stats["Simulation Data"][-1][5],  # Latest latency value
        "Hop Reduction": stats["Simulation Data"][-1][3]  # Latest hop reduction value
    })

# Find the best policy based on different metrics
best_cache_hit_policy = max(policy_stats, key=lambda x: x["Cache Hit Ratio"])
best_latency_policy = min(policy_stats, key=lambda x: x["Latency"])
best_hop_reduction_policy = max(policy_stats, key=lambda x: x["Hop Reduction"])

print("\nBest Policies Based on Performance:")
print(f" Best Cache Hit Ratio Policy: {best_cache_hit_policy['Policy']} ({best_cache_hit_policy['Cache Hit Ratio']:.2f}%)")
print(f" Lowest Latency Policy: {best_latency_policy['Policy']} ({best_latency_policy['Latency']:.4f} sec)")
print(f" Best Hop Reduction Policy: {best_hop_reduction_policy['Policy']} ({best_hop_reduction_policy['Hop Reduction']:.2f})")
...
ContentIDManager.initialize_index(publishers)
    ...
    # Set up the Forwarding Information Base (FIB) for each router
